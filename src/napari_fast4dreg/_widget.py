...
def parse_axis_order(axis_string):
    ...
    if not all(c in valid_chars for c in axis_string):
        raise ValueError(f"Invalid axis order: {axis_string}. Use only C, T, Z, Y, X")
...
def convert_to_ctzyx(img, axis_string):
    ...
    # Build transformation based on input dimensionality
    # We need to:
    # 1. Add singleton dims for missing axes
    # 2. Reorder axes to CTZYX
...
def revert_to_original_axis_order(data, original_axis_string):
    ...
    Removes added singleton dimensions and reorders axes back to user's original specification.
    
    Args:
        data: Data in CTZYX format (5D dask array)
        original_axis_string: Original axis order provided by user (e.g., "TZYX", "ZYX")
    
    Returns:
        Data reordered to match original axis string and with singleton dims removed
    """
    ...
    # Build mapping of original axis order
